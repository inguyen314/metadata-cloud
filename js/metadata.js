document.addEventListener('DOMContentLoaded', async function () {
    ...
    if (reportNumber === 1) {
        ...
        setLookForward = addDaysFromDate(new Date(), 0);
    }
...
function createTableMetadataLatLong(combinedDataRiver, type, reportNumber) {
    // Create a table element and set an ID for styling or selection purposes
    const table = document.createElement('table');
    table.setAttribute('id', 'webrep');
    ...
    combinedDataRiver.forEach((basin) => {
        ...
        basin['assigned-locations'].forEach((location) => {
            ...
            locCell.textContent = location && location['location-id']
                ? location['location-id'].split('-')[0]
                : 'N/A';
            // locCell.style.width = '50%';
            // locCell.style.cursor = 'pointer';
            // locCell.style.textDecoration = 'underline'; // Add underline styling

            // // Add click event to locCell to open Google Maps link
            // locCell.addEventListener('click', () => {
            //     const latitude = location?.metadata?.latitude;
            //     const longitude = location?.metadata?.longitude;

            //     if (latitude && longitude) {
            //         const googleMapsUrl = `https://www.google.com/maps?&z=20&mrt=yp&t=k&q=${latitude}+${longitude}`;
            //         window.open(googleMapsUrl, '_blank');
            //     } else {
            //         alert("Latitude or Longitude data is missing.");
            //     }
            // });
            ...
            latCell.textContent = location && location['metadata'] && location['metadata']['latitude']
                ? location['metadata']['latitude']
                : 'N/A';
            // latCell.style.width = '25%';
            latCell.style.cursor = 'pointer';
            latCell.style.textDecoration = 'underline'; // Add underline styling

            // Add click event to latCell to open Google Maps link
            latCell.addEventListener('click', () => {
                const latitude = location?.metadata?.latitude;
                const longitude = location?.metadata?.longitude;
            ...
            longCell.textContent = location && location['metadata'] && location['metadata']['longitude']
                ? location['metadata']['longitude']
                : 'N/A';
            // longCell.style.width = '25%';
            longCell.style.cursor = 'pointer';
            longCell.style.textDecoration = 'underline'; // Add underline styling

            // Add click event to longCell to open Google Maps link
            longCell.addEventListener('click', () => {
                const latitude = location?.metadata?.latitude;
                const longitude = location?.metadata?.longitude;
            ...
            countyCell.textContent = location && location['metadata'] && location['metadata']['county-name']
                ? location['metadata']['county-name']
                : 'N/A';
            row.appendChild(countyCell);

            // 04 - State
            const stateCell = document.createElement('td');
            stateCell.textContent = location && location['metadata'] && location['metadata']['state-initial']
...
function createTableMetadataData(combinedDataRiver, type, reportNumber) {
    ...
    basin['assigned-locations'].forEach((location) => {
        ...
        // Add header row with colspan 2 for recordStage
        const recordStageHeaderRow = document.createElement('tr');
        const recordStageHeaderCell = document.createElement('th');
        recordStageHeaderCell.colSpan = 2;
        recordStageHeaderCell.textContent = 'Record Stage';
        recordStageHeaderCell.style.textAlign = 'center'; // Center-align the header
        recordStageHeaderRow.appendChild(recordStageHeaderCell);
        table.appendChild(recordStageHeaderRow);

        // Iterate through the recordStage object if it exists, and display key-value pairs in two columns
        const recordStage = location[`record-stage`];
        if (recordStage) {
            for (const [key, value] of Object.entries(recordStage)) {
                const recordStageRow = document.createElement('tr');

                // Key cell for recordStage
                const keyCell = document.createElement('td');
                keyCell.textContent = key;
                keyCell.style.fontWeight = 'bold';
                keyCell.style.width = '25%';  // Set width to 25%
                keyCell.style.textAlign = 'left';  // Align text to the left
                recordStageRow.appendChild(keyCell);

                // Value cell for recordStage
                const valueCell = document.createElement('td');
                valueCell.textContent = value;
                valueCell.style.width = '75%';  // Set width to 75%
                valueCell.style.textAlign = 'left';  // Align text to the left
                recordStageRow.appendChild(valueCell);

                // Append the recordStage row to the table
                table.appendChild(recordStageRow);
            }
        }
        
        // Add header row with colspan 2 for lwrp
        const lwrpHeaderRow = document.createElement('tr');
        const lwrpHeaderCell = document.createElement('th');
        lwrpHeaderCell.colSpan = 2;
        lwrpHeaderCell.textContent = 'LWRP';
        lwrpHeaderCell.style.textAlign = 'center'; // Center-align the header
        lwrpHeaderRow.appendChild(lwrpHeaderCell);
        table.appendChild(lwrpHeaderRow);

        // Iterate through the lwrp object if it exists, and display key-value pairs in two columns
        const lwrp = location.lwrp;
        if (lwrp) {
            for (const [key, value] of Object.entries(lwrp)) {
                const lwrpRow = document.createElement('tr');
                ...
                valueCell.style.width = '75%';  // Set width to 75%
                valueCell.style.textAlign = 'left';  // Align text to the left
                lwrpRow.appendChild(valueCell);

                // Append the lwrp row to the table
                table.appendChild(lwrpRow);
            }
        }
...
function updateLocDataMetadata(locData, type, data, lastValue) {
    const keys = {
        apiDataKey: `${type}-api-data`,
        lastValueKey: `${type}-last-value`
    };
